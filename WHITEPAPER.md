# brindavanchat Protocol Whitepaper
...
brindavanchat is a decentralized, peer-to-peer messaging application designed for secure, private, and censorship-resistant communication over ephemeral, ad-hoc networks. This whitepaper details the brindavanchat Protocol Stack, a layered architecture that combines a modern cryptographic foundation with a flexible application protocol. At its core, brindavanchat leverages the Noise Protocol Framework (specifically, the `XX` pattern) to establish mutually authenticated, end-to-end encrypted sessions between peers. This document provides a technical specification of the identity management, session lifecycle, message framing, and security considerations that underpin the brindavanchat network.

---

## 1. Introduction

In an era of centralized communication platforms, brindavanchat offers a resilient alternative by operating without central servers. It is designed for scenarios where internet connectivity is unavailable or untrustworthy, such as protests, natural disasters, or remote areas. Communication occurs directly between devices over transports like Bluetooth Low Energy (BLE).

The design goals of the brindavanchat Protocol are:
...
The brindavanchat Protocol is a four-layer stack. This layered approach separates concerns, allowing for modularity and future extensibility.
...
graph TD
    ...
    subgraph "brindavanchat Application"
        A
    end
...
*   **Application Layer:** Defines the structure of user-facing messages (`brindavanchatMessage`), acknowledgments (`DeliveryAck`), and other application-level data.
*   **Session Layer:** Manages the overall communication packet (`brindavanchatPacket`). This includes routing information (TTL), message typing, fragmentation, and serialization into a compact binary format.
...
A peer's identity in brindavanchat is defined by two persistent cryptographic key pairs, which are generated on first launch and stored securely in the device's Keychain.
...
Beyond cryptographic identity, brindavanchat incorporates a social trust layer, allowing users to manage their relationships with peers. This functionality is handled by the `SecureIdentityStateManager`.
...
brindavanchat implements the Noise Protocol Framework to provide strong, authenticated end-to-end encryption.
...
## 6. The brindavanchat Session and Application Protocol

Once a Noise session is established, peers exchange `brindavanchatPacket` structures, which are encrypted as the payload of Noise transport messages.

### 6.1. Binary Packet Format (`brindavanchatPacket`)

To minimize bandwidth, `brindavanchatPacket`s are serialized into a compact binary format. The structure is designed to be fixed-size where possible to resist traffic analysis.
...
```
_A representation of the sizes of the fields in `brindavanchatPacket`_

### 6.2. Application Message Format (`brindavanchatMessage`)

For packets of type `message`, the payload is a binary-serialized `brindavanchatMessage` containing the chat content.
...
```
_A representation of the sizes of the fields in `brindavanchatMessage`_

---

## 7. Message Routing and Propagation

brindavanchat operates as a decentralized mesh network, meaning there are no central servers to route messages. Packets are propagated through the network from peer to peer. The protocol supports several modes of message delivery.
...
Transport layers like BLE have a Maximum Transmission Unit (MTU) that limits the size of a single packet. To handle messages larger than this limit, brindavanchat implements a fragmentation protocol.
...
The brindavanchat Protocol provides a robust and secure foundation for decentralized, peer-to-peer communication. By layering a flexible application protocol on top of the well-regarded Noise Protocol Framework, it achieves strong confidentiality, authentication, and forward secrecy. The use of a compact binary format and thoughtful security considerations like rate limiting and traffic analysis resistance make it suitable for use in challenging network environments.
