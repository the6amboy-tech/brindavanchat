...
struct PublicTimelineStore {
    private var meshTimeline: [brindavanchatMessage] = []
    private var geohashTimelines: [String: [brindavanchatMessage]] = [:]
    ...
    mutating func append(_ message: brindavanchatMessage, to channel: ChannelID) {
        switch channel {
        case .mesh:
    ...
    mutating func append(_ message: brindavanchatMessage, toGeohash geohash: String) {
    ...
    mutating func appendIfAbsent(_ message: brindavanchatMessage, toGeohash geohash: String) -> Bool {
        var timeline = geohashTimelines[geohash] ?? []
    ...
    mutating func messages(for channel: ChannelID) -> [brindavanchatMessage] {
        switch channel {
        case .mesh:
            return meshTimeline
        case .location(let channel):
    ...
    @discardableResult
    mutating func removeMessage(withID id: String) -> brindavanchatMessage? {
    ...
    mutating func removeMessages(in geohash: String, where predicate: (brindavanchatMessage) -> Bool) {
        var timeline = geohashTimelines[geohash] ?? []
        timeline.removeAll(where: predicate)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }

    mutating func mutateGeohash(_ geohash: String, _ transform: (inout [brindavanchatMessage]) -> Void) {
        var timeline = geohashTimelines[geohash] ?? []
        transform(&timeline)
        geohashTimelines[geohash] = timeline.isEmpty ? nil : timeline
    }
    ...
    private func trimGeohashTimelineIfNeeded(_ timeline: inout [brindavanchatMessage]) {
